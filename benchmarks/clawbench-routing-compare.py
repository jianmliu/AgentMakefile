...
def render_markdown(tasks: List[Dict[str, str]], rows: Dict[str, Dict[str, Dict]]) -> str:
    ...
    lines.append("")
    lines.append("Each row measures the cumulative improvement on the 6 OpenClaw-")
    lines.append("domain tasks (the 4 methodology tasks always route correctly via")
    lines.append("the root AgentMakefile).")
    lines.append("")
    ...
    lines.append("| + closed-loop feedback (demos/evo-feedback-loop-demo/run.py) | **6 / 6** | 6 / 6 |")
    lines.append("")
