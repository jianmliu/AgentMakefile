...
def render_markdown(tasks: List[Dict[str, str]], rows: Dict[str, Dict[str, Dict]]) -> str:
    ...
    lines.append("")
    lines.append("Outstanding gap (still 4/6 OpenClaw tasks routing wrong on top-1):")
    lines.append("")
    lines.append("- `bundled-1password`, `bundled-apple-notes`, `plugins-spreadsheet`,")
    lines.append("  `vendor-stripe`. The detector + patch infrastructure to close")
    lines.append("  these is in place; what's still missing is the user_feedback /")
    lines.append("  benchmark evidence that drives the detectors. The next concrete")
    lines.append("  step is to run real tasks against the corpus, capture")
    lines.append("  selected_target outcomes as plugin_payload evidence, and feed")
    lines.append("  corrections back as user_feedback. The closed loop will then")
    lines.append("  prune the broad neighbour terms and attach distinguishing phrases")
    lines.append("  on the right skills automatically.")
    return "\n".join(lines) + "\n"
