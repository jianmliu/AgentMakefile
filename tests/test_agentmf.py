...
# ---------------------------------------------------------------------------
# `agentmf configure` — status / list / validate / add / remove for the
# `compile.targets` build set. Mirrors the GNU autotools `./configure` role
# (build-time backend selection) rather than runtime per-request routing.
# ---------------------------------------------------------------------------


def _write_minimal_configure_makefile(
    tmp_path: Path,
    targets: List[str] = ["claude-md", "agents-md"],
) -> Path:
    path = tmp_path / "AgentMakefile"
    body = (
        "version: '0.1'\n"
        "metadata:\n  name: configure-test\n"
        "compile:\n  targets:\n" + "".join(f"    - {t}\n" for t in targets) +
...
def test_configure_status_shows_active_backends_and_paths(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(tmp_path, targets=["claude-md", "agents-md"])
    result = create_configure_payload(path=path, action="status")

    assert result.ok, result.diagnostics.format()
    payload = result.payload
    assert payload["mode"] == "configure"
    assert payload["file"] == str(path)
    active = payload["active_backends"]
    by_name = {entry["backend"]: entry for entry in active}
    assert sorted(by_name) == ["agents-md", "claude-md"]
    assert by_name["claude-md"]["path"] == "CLAUDE.md"
    assert by_name["agents-md"]["path"] == "AGENTS.md"
    # Sanity: every supported backend not in active list shows up as available.
    assert "opencode" in payload["available_backends"]
    assert "claude-md" not in payload["available_backends"]


def test_configure_status_marks_default_when_compile_targets_is_empty(tmp_path: Path) -> None:
    """If compile.targets is empty/missing, the compiler falls back to
    DEFAULT_BACKENDS. Status must reflect that with an `is_default` marker
    so users can distinguish "explicitly chose these" from "got the
    fallback set".
    """
    from agentmf.configure import create_configure_payload
    ...
    assert result.ok, result.diagnostics.format()
    assert result.payload["is_default"] is True
    by_name = {entry["backend"]: entry for entry in result.payload["active_backends"]}
    # DEFAULT_BACKENDS = ["claude-md", "agents-md", "cursor-rule"]
    assert sorted(by_name) == ["agents-md", "claude-md", "cursor-rule"]


def test_configure_list_returns_all_supported_backends(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(tmp_path)
    result = create_configure_payload(path=path, action="list")

    assert result.ok, result.diagnostics.format()
    backends = {entry["backend"]: entry for entry in result.payload["backends"]}
    # All registered backends appear.
    for name in ("claude-md", "agents-md", "cursor-rule", "claude-skill",
                 "codex-skill", "skills-index", "claude-code", "opencode",
                 "agents-fragments", "claude-fragments"):
        assert name in backends, f"missing backend {name!r}"
    # Each entry has at least a description + default output hint.
    for entry in backends.values():
        assert isinstance(entry.get("description"), str) and entry["description"]
        assert "default_output" in entry


def test_configure_validate_flags_unknown_backend_in_compile_targets(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(
        tmp_path, targets=["claude-md", "not-a-real-backend"]
    )
    result = create_configure_payload(path=path, action="validate")

    assert not result.ok
    codes = {item.code for item in result.diagnostics.items}
    assert "AMF103" in codes, codes  # same code compile uses for unknown backend


def test_configure_add_appends_backend_idempotently(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(tmp_path, targets=["claude-md"])

    first = create_configure_payload(path=path, action="add", backend="opencode", write=True)
    assert first.ok, first.diagnostics.format()
    assert first.payload["wrote"] is True
    assert "opencode" in first.payload["compile_targets"]

    # Idempotent: re-adding does not duplicate and reports wrote=False.
    second = create_configure_payload(path=path, action="add", backend="opencode", write=True)
    assert second.ok
    assert second.payload["wrote"] is False
    assert second.payload["compile_targets"].count("opencode") == 1

    # File on disk reflects the addition and remains a parseable AgentMakefile.
    from agentmf.loader import load_source
    reloaded = load_source(path)
    assert "opencode" in reloaded.compile.targets
    assert "claude-md" in reloaded.compile.targets


def test_configure_add_rejects_unknown_backend(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(tmp_path)
    result = create_configure_payload(path=path, action="add", backend="bogus", write=True)

    assert not result.ok
    codes = {item.code for item in result.diagnostics.items}
    assert "AMF103" in codes


def test_configure_remove_drops_backend_and_is_idempotent(tmp_path: Path) -> None:
    from agentmf.configure import create_configure_payload

    path = _write_minimal_configure_makefile(tmp_path, targets=["claude-md", "agents-md"])

    first = create_configure_payload(path=path, action="remove", backend="agents-md", write=True)
    assert first.ok, first.diagnostics.format()
    assert first.payload["wrote"] is True
    assert "agents-md" not in first.payload["compile_targets"]
    assert "claude-md" in first.payload["compile_targets"]

    # Removing again is a no-op success.
    second = create_configure_payload(path=path, action="remove", backend="agents-md", write=True)
    assert second.ok
    assert second.payload["wrote"] is False


def test_cli_configure_status_text(tmp_path: Path, capsys) -> None:
    path = _write_minimal_configure_makefile(tmp_path, targets=["claude-md", "opencode"])

    exit_code = main(["configure", "--file", str(path)])

    assert exit_code == 0
    out = capsys.readouterr().out
    assert "claude-md" in out
    assert "opencode" in out
    assert "CLAUDE.md" in out  # path for claude-md
    assert "opencode.json" in out  # default path for opencode


def test_cli_configure_add_writes_file(tmp_path: Path, capsys) -> None:
    path = _write_minimal_configure_makefile(tmp_path, targets=["claude-md"])

    exit_code = main([
        "configure", "--file", str(path),
        "--add", "opencode", "--write", "--format", "json",
    ])
    assert exit_code == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["ok"] is True
    assert "opencode" in payload["compile_targets"]
