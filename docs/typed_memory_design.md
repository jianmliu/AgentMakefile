...
| **M5** | read side: `select` returns task-scoped, **kind-filtered** memory bundles; **kind-aware compile rendering** (procedural → actionable skill file; semantic/episodic → `MEMORY.md` line / context bundle), now that `kind` is in the IR. |
