...
| **M5 — landed (read side)** | `agentmf.memory_recall.recall_memory(path, request, kinds=…)` routes a request against a scanned `MemoryMakefile` with the existing matcher, **filters the selected units by `kind`**, and renders a **kind-aware bundle** (procedural → `apply <name> — …` actionable; semantic/episodic → context line). Reuses the selector — no backend changes, zero compile-test risk. Proven end-to-end: corpus → scan → `recall_memory`. (Per-kind rendering inside the compile *backends* — claude-skill etc. — stays optional; the recall bundle already delivers the kind-aware read view.) |
